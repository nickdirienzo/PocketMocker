 ...
 * <p>
 * You do not instantiate this class directly; instead, you retrieve a reference
 * to an instance through {@link android.content.Context#getSystemService
 * Context.getSystemService(Context.TELEPHONY_SERVICE)}.
 * <p>
 * Note that access to some telephony information is permission-protected. Your
 * application cannot access the protected information unless it has the
 * appropriate permissions declared in its manifest file. Where permissions
 * apply, they are noted in the the methods through which you access the
...
public class TelephonyManager {
    ...
    private static final String PM_PACKAGE = "edu.buffalo.cse.blue.pocketmocker";
    private Messenger mMockListener;
    private ServiceConnection mMockServiceConnection;
    private Messenger mMockSubscriber;
    private boolean mIsReplaying = false;
    private CdmaCellLocation mMockCdmaCellLocation;

    private class MockCellLocationListener extends Handler {
        @Override
        public void handleMessage(Message msg) {
            if (!sContext.getPackageName().equals(PM_PACKAGE)) {
                ...
                if (mIsReplaying) {
                    ...
                    mMockCdmaCellLocation = new CdmaCellLocation(data);
                }
            }
        }
    }
    ...
    public TelephonyManager(Context context) {
        if (sContext == null) {
            ...
            mMockListener = new Messenger(new MockCellLocationListener());
    ...
    private void bindToMockerService() {
        ...
        if (sContext != null) {
            ...
            sContext.bindService(new Intent(PM_PACKAGE + ".MockerService"),
    ...
    /**
     * @hide /* @deprecated - use getSystemService as described above
     */
    ...
    /**
     * Broadcast intent action indicating that the call state (cellular) on the
     * device has changed.
     * <p>
     * The {@link #EXTRA_STATE} extra indicates the new call state. If the new
     * state is RINGING, a second extra {@link #EXTRA_INCOMING_NUMBER} provides
     * the incoming phone number as a String.
     * <p class="note">
     * Requires the READ_PHONE_STATE permission.
     * <p class="note">
     * This was a {@link android.content.Context#sendStickyBroadcast sticky}
     * broadcast in version 1.0, but it is no longer sticky. Instead, use
     * {@link #getCallState} to synchronously query the current call state.
     * 
     * @see #EXTRA_STATE
    ...
    /**
     * The lookup key used with the {@link #ACTION_PHONE_STATE_CHANGED}
     * broadcast for a String containing the new call state.
     * 
     * @see #EXTRA_STATE_IDLE
     * @see #EXTRA_STATE_RINGING
     * @see #EXTRA_STATE_OFFHOOK <p class="note">
    ...
    /**
     * The lookup key used with the {@link #ACTION_PHONE_STATE_CHANGED}
     * broadcast for a String containing the incoming phone number. Only valid
     * when the new call state is RINGING.
     * <p class="note">
     * Retrieve with {@link android.content.Intent#getStringExtra(String)}.
     */
    ...
    /**
     * Returns the current location of the device. Return null if current
    ...
    public CellLocation getCellLocation() {
        if(mIsReplaying && !sContext.getPackageName().equals(PM_PACKAGE)) {
            Log.v(PM_TAG, "Returning mock!");
            return mMockCdmaCellLocation;
        }
        try {
     ...
     * PhoneStateListener.onCellLocationChanged} will be called on location
     * updates.
     * <p>
     * Requires Permission:
     * {@link android.Manifest.permission#CONTROL_LOCATION_UPDATES
    ...
    /**
     * Returns the current phone type. TODO: This is a last minute change and
     * hence hidden.
     * 
     * @see #PHONE_TYPE_NONE
     * @see #PHONE_TYPE_GSM
     * @see #PHONE_TYPE_CDMA
     * @see #PHONE_TYPE_SIP {@hide}
     */
    ...
    /**
     * This function returns the type of the phone, depending on the network
     * mode.
     * 
     * @param network mode
     * @return Phone Type
    ...
    /**
     * Return if the current radio is LTE on CDMA. This is a tri-state return
     * value as for a period of time the mode may be unknown.
     * 
     * @return {@link PhoneConstants#LTE_ON_CDMA_UNKNOWN},
     *         {@link PhoneConstants#LTE_ON_CDMA_FALSE} or
     ...
     * value as for a period of time the mode may be unknown.
     * 
     * @return {@link Phone#LTE_ON_CDMA_UNKNOWN},
     *         {@link Phone#LTE_ON_CDMA_FALSE} or {@link Phone#LTE_ON_CDMA_TRUE}
     * @hide
     */
     ...
     * READ_PHONE_STATE}
     * 
     * @hide nobody seems to call this.
     */
    ...
    /**
     * Returns the MSISDN string. for a GSM phone. Return null if it is
     * unavailable.
     * <p>
     * Requires Permission: {@link android.Manifest.permission#READ_PHONE_STATE
     * READ_PHONE_STATE}
    ...
    /**
     * Retrieves the alphabetic identifier associated with the voice mail
     * number.
     * <p>
     * Requires Permission: {@link android.Manifest.permission#READ_PHONE_STATE
     * READ_PHONE_STATE}
    ...
    /**
     * Returns the IMS private user identity (IMPI) that was loaded from the
     * ISIM.
     * 
    ...
    /**
     * Data connection activity: Currently both sending and receiving IP PPP
     * traffic.
     */
    ...
    /**
     * Data connection state: Suspended. The connection is up, but IP traffic is
     * temporarily unavailable. For example, in a 2G network, data activity may
     ...
     * <p>
     * To register a listener, pass a {@link PhoneStateListener} and specify at
     * least one telephony state of interest in the events argument. At
     * registration, and when a specified telephony state changes, the telephony
     * manager invokes the appropriate callback method on the listener object
     * and passes the current (udpated) values.
     * <p>
     * To unregister a listener, pass the listener object and set the events
     * argument to {@link PhoneStateListener#LISTEN_NONE LISTEN_NONE} (0).
     * 
     * @param listener The {@link PhoneStateListener} object to register (or
     *            unregister)
     * @param events The telephony state(s) of interest to the listener, as a
     *            bitwise-OR combination of {@link PhoneStateListener} LISTEN_
     *            flags.
     */
     ...
     *         active. This will be false on "data only" devices which can't
     *         make voice calls and don't support any in-call UI.
     *         <p>
