...
public class SystemSensorManager extends SensorManager {
    ...
    /*
     * The thread and the sensor list are global to the process but the actual
     * thread is spawned on demand
     */
    ...
    private final String PM_TAG = "PM_" + TAG;
                ...
                while (true) {
                    ...
                    synchronized (sListeners) {
                        if (sensor == -1 || sListeners.isEmpty()) {
                            // we lost the connection to the event stream. this
                            // happens
                            // when the last listener is removed or if there is
                            // an error
                            if (sensor == -1 && !sListeners.isEmpty()) {
                                // log a warning in case of abnormal termination
                                Log.e(TAG, "_sensors_data_poll() failed, we bail out: sensors="
                                        + sensor);
                            }
                            // we have no more listeners or polling failed,
    ...
    private class ListenerDelegate {
        ...
        ListenerDelegate(SensorEventListener listener, Sensor sensor, Handler handler) {
            ...
            // currently we create one Handler instance per listener, but we
            // could
                ...
                public void handleMessage(Message msg) {
                    ...
                    // Only report accuracy for sensors that support it.
                        ...
                        case Sensor.TYPE_ORIENTATION:
                            // call onAccuracyChanged() only if the value
                            // changes
    ...
    public SystemSensorManager(Context context, Looper mainLooper) {
        mContext = context;
        Log.v(PM_TAG, "Context: " + context.getPackageName());
        mMainLooper = mainLooper;
        Log.v(PM_TAG, "Creating mMockListener");
        mMockListener = new Messenger(new MockSensorEventListener());
