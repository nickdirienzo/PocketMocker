 ...
 * {@link android.Manifest.permission#ACCESS_FINE_LOCATION} permissions. If your
 * application only has the coarse permission then it will not have access to
 * the GPS or passive location providers. Other providers will still return
 * location results, but the update rate will be throttled and the exact
 * location will be obfuscated to a coarse level of accuracy.
 * 
 * @param <ServiceConnection>
 */
public class LocationManager<ServiceConnection> {
    ...
    // Listens for updates from edu.buffalo.cse.blue.pocketmocker.MockerService.
    private Messenger mMockListener;
    // The next two fields are used to establish a connection to MockerService.
    private android.content.ServiceConnection mMockServiceConnection;
     ...
     * other applications or services request them without actually requesting
     * the locations yourself. This provider will return locations generated by
     * other providers. You can query the {@link Location#getProvider()} method
     * to determine the origin of the location update. Requires the permission
     * {@link android.Manifest.permission#ACCESS_FINE_LOCATION}, although if the
     * GPS is not enabled this provider might only return coarse fixes.
     */
     ...
     * <p>
     * This provider combines inputs for all possible location sources to
     * provide the best possible Location fix. It is implicitly used for all
     * API's that involve the {@link LocationRequest} object.
     * 
     * @hide
     */
    ...
    private class ListenerTransport extends ILocationListener.Stub {
        ...
        // nvdirien: Every action passed from the ILocationListener IPC
        // ultimately is handled here. All we need to do is check to see if we
        // are currently in replay-mode.
    ...
    // Start nvdirien
    private class MockLocationListener extends Handler {
        @Override
        public void handleMessage(Message msg) {
    ...
    public LocationManager(Context context, ILocationManager service) {
        ...
        mMockServiceConnection = new android.content.ServiceConnection() {
     ...
     * <p>
     * All providers are returned, including ones that are not permitted to be
     ...
     * @param enabledOnly if true then only a provider that is currently enabled
     *            is returned
     ...
     * @throws IllegalArgumentException if provider is null or doesn't exist on
     *             this device
     ...
     *            implement the callback mechanism, or null to make callbacks on
     *            the calling thread
    ...
    /**
     * Register for location updates using a Criteria, and a callback on the
     * specified looper thread.
     * <p>
     * See {@link #requestLocationUpdates(long, float, Criteria, PendingIntent)}
     * for more detail on how to use this method.
     * 
     ...
     *            milliseconds
     * @param minDistance minimum distance between location updates, in meters
     * @param criteria contains parameters for the location manager to choose
     *            the appropriate provider and parameters to compute the
     *            location
     * @param listener a {@link LocationListener} whose
     *            {@link LocationListener#onLocationChanged} method will be
     *            called for each location update
     ...
     * <p>
     * It may take a while to receive the first location update. If an immediate
     ...
     * <p>
     * If the caller supplied a pending intent, then location updates are sent
     * with a key of {@link #KEY_LOCATION_CHANGED} and a
     * {@link android.location.Location} value.
     * <p>
     * The location update interval can be controlled using the minTime
     * parameter. The elapsed time between location updates will never be less
     * than minTime, although it can be more depending on the Location Provider
     * implementation and the update interval requested by other applications.
     * <p>
     * Choosing a sensible value for minTime is important to conserve battery
     * life. Each location update requires power from GPS, WIFI, Cell and other
     * radios. Select a minTime value as high as possible while still providing
     * a reasonable user experience. If your application is not in the
     * foreground and showing location to the user then your application should
     * avoid using an active provider (such as {@link #NETWORK_PROVIDER} or
     * {@link #GPS_PROVIDER}), but if you insist then select a minTime of 5 * 60
     * * 1000 (5 minutes) or greater. If your application is in the foreground
     * and showing location to the user then it is appropriate to select a
     * faster update interval.
     * <p>
     * The minDistance parameter can also be used to control the frequency of
     * location updates. If it is greater than 0 then the location provider will
     * only send your application an update when the location has changed by at
     * least minDistance meters, AND at least minTime milliseconds have passed.
     * However it is more difficult for location providers to save power using
     * the minDistance parameter, so minTime should be the primary tool to
     * conserving battery life.
     * <p>
     * If your application wants to passively observe location updates triggered
     * by other applications, but not consume any additional power otherwise,
     * then use the {@link #PASSIVE_PROVIDER} This provider does not actively
     * turn on or modify active location providers, so you do not need to be as
     * careful about minTime and minDistance. However if your application
     * performs heavy work on a location update (such as network activity) then
     * you should select non-zero values for minTime and/or minDistance to
     * rate-limit your update frequency in the case another application enables
     * a location provider with extremely fast updates.
     * <p>
     * In case the provider is disabled by the user, updates will stop, and a
     * provider availability update will be sent. As soon as the provider is
     * enabled again, location updates will immediately resume and a provider
     * availability update sent. Providers can also send status updates, at any
     * time, with extra's specific to the provider. If a callback was supplied
     ...
     * {@link LocationListener#onStatusChanged}. Alternately, if a pending
     * intent was supplied then status and availability updates are broadcast
     * intents with extra keys of {@link #KEY_PROVIDER_ENABLED} or
     * {@link #KEY_STATUS_CHANGED}.
     * <p>
     * If a {@link LocationListener} is used but with no Looper specified then
     * the calling thread must already be a {@link android.os.Looper} thread
     * such as the main thread of the calling Activity. If a Looper is specified
     * with a {@link LocationListener} then callbacks are made on the supplied
     * Looper thread.
     * <p class="note">
     * Prior to Jellybean, the minTime parameter was only a hint, and some
     * location provider implementations ignored it. From Jellybean and onwards
     * it is mandatory for Android compatible devices to observe both the
     * minTime and minDistance parameters.
     * 
     * @param minTime minimum time interval between location updates, in
     *            milliseconds
     * @param minDistance minimum distance between location updates, in meters
    ...
    /**
     * Register for a single location update using a Criteria and pending
     * intent.
     * <p>
     * See {@link #requestLocationUpdates(long, float, Criteria, PendingIntent)}
     * for more detail on how to use this method.
     ...
     * <p>
     * Upon a location update, the system delivers the new {@link Location} to
     * the provided {@link LocationListener}, by calling its
     * {@link LocationListener#onLocationChanged} method.
     * </p>
     * <p>
     * The system will automatically select and enable the best providers to
     * compute a location for your application. It may use only passive
     * locations, or just a single location source, or it may fuse together
     * multiple location sources in order to produce the best possible result,
     * depending on the quality of service requested in the
     ...
     * receive location updates as available, without a major power impact on
     * the system. If your application just needs an occasional location update
     * without any strict demands, then pass a null LocationRequest.
     * <p>
     * Only one LocationRequest can be registered for each unique callback or
     * pending intent. So a subsequent request with the same callback or pending
     * intent will over-write the previous LocationRequest.
     * <p>
     * If a pending intent is supplied then location updates are sent with a key
     * of {@link #KEY_LOCATION_CHANGED} and a {@link android.location.Location}
     * value. If a callback is supplied then location updates are made using the
     * {@link LocationListener#onLocationChanged} callback, on the specified
     * Looper thread. If a {@link LocationListener} is used but with a null
     * Looper then the calling thread must already be a
     * {@link android.os.Looper} thread (such as the main thread) and callbacks
     * will occur on this thread.
     * <p>
     * Provider status updates and availability updates are deprecated because
     * the system is performing provider fusion on the applications behalf. So
     * {@link LocationListener#onProviderDisabled},
     * {@link LocationListener#onProviderEnabled},
     * {@link LocationListener#onStatusChanged} will not be called, and intents
     * with extra keys of {@link #KEY_PROVIDER_ENABLED} or
     * {@link #KEY_STATUS_CHANGED} will not be received.
     * <p>
     * To unregister for Location updates, use:
     * {@link #removeUpdates(LocationListener)}.
     * 
    ...
    /**
     * Register for fused location updates using a LocationRequest and a pending
     * intent.
     * <p>
     * Upon a location update, the system delivers the new {@link Location} with
     * your provided {@link PendingIntent}, as the value for
     * {@link LocationManager#KEY_LOCATION_CHANGED} in the intent's extras.
     * </p>
     * <p>
     * To unregister for Location updates, use:
     * {@link #removeUpdates(PendingIntent)}.
     * <p>
     * See
     * {@link #requestLocationUpdates(LocationRequest, LocationListener, Looper)}
     * for more detail.
     * 
     * @param request quality of service required, null for default low power
     * @param intent a {@link PendingIntent} to be sent for the location update
    ...
    private ListenerTransport wrapListener(LocationListener listener, Looper looper) {
        ...
        synchronized (mListeners) {
            ...
            // nvdirien: All LocationListeners are keys in mListeners, so to
            // send a new mock location, we just broadcast to all the values.
     ...
     * <p>
     * When the device detects that it has entered or exited the area
     * surrounding the location, the given PendingIntent will be used to create
     * an Intent to be fired.
     * <p>
     ...
     * {@link android.Manifest.permission#ACCESS_COARSE_LOCATION}. From API
     * version 17 and onwards, this method requires
     ...
     * @param radius the radius of the central point of the alert region, in
     *            meters
     * @param expiration time for this proximity alert, in milliseconds, or -1
     ...
     * entering the proximity region; if false, it is exiting.
     * <p>
     * The geofence engine fuses results from all location providers to provide
     * the best balance between accuracy and power. Applications can choose the
     * quality of service required using the {@link LocationRequest} object. If
     * it is null then a default, low power geo-fencing implementation is used.
     * It is possible to cross a geo-fence without notification, but the system
     * will do its best to detect, using {@link LocationRequest} as a hint to
     * trade-off accuracy and power.
     * <p>
     ...
     * <p>
     * This removes only the specified geofence associated with the specified
     * pending intent. All other geofences remain unchanged.
     * 
     ...
     * <p>
     * This can be done without starting the provider. Note that this location
     * could be out-of-date, for example if the device was turned off and moved
     * to another location.
     * <p>
    ...
    // TODO: It would be fantastic to deprecate mock providers entirely, and
    // replace
     ...
     * 
     * @param name the provider name
     * @throws SecurityException if the ACCESS_MOCK_LOCATION permission is not
     *             present or the
     *             {@link android.provider.Settings.Secure#ALLOW_MOCK_LOCATION
     *             Settings.Secure.ALLOW_MOCK_LOCATION} system setting is not
     *             enabled
     * @throws IllegalArgumentException if a provider with the given name
     *             already exists
     */
     ...
     *             Settings.Secure.ALLOW_MOCK_LOCATION} system setting is not
     *             enabled
     * @throws IllegalArgumentException if no provider with the given name
     *             exists
     */
     ...
     * <p>
     * This location will be used in place of any actual location from the
     * provider. The location object must have a minimum number of fields set to
     * be considered a valid LocationProvider Location, as per documentation on
     * {@link Location} class.
     * 
     * @param provider the provider name
     ...
     * @throws SecurityException if the ACCESS_FINE_LOCATION permission is not
     *             present
     */
     ...
     * data is copied atomically. The caller may either pass in a
     * {@link GpsStatus} object to set with the latest status information, or
     ...
     * @param extras optional arguments for the command (or null). The provider
     *            may optionally fill the extras Bundle with results from the
     *            command.
